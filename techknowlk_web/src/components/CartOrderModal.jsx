...
export default function CartOrderModal({ cartItems, cartTotal, isOpen, onClose, onClearCart }) {
  ...
  const handleWhatsApp = async (e) => {
    ...
    try {
      await fetch('/api/save-order', {
        ...
        body: JSON.stringify({
          ...
          total: cartTotal ? cartTotal.toString() : null,
        })
      });
    } catch (err) {
    ...
    const msg =
      `Hello TechKnowLK! I'd like to place an order from my cart:\n\n` +
      `🛒 Items Ordered:\n${itemsLines.join('\n')}${totalLine}\n\n` +
      `👤 Customer Details:\n` +
      `Name: ${form.name}\n` +
      `Phone: ${form.phone}\n` +
    ...
    try {
      const res = await fetch('/api/send-cart-order-email', {
        ...
        body: JSON.stringify({
          ...
          total: cartTotal ? cartTotal.toLocaleString() : null,
        }),
      });
  ...
  return (
    <>
      ...
      >
        <div
          data-lenis-prevent
          className="relative w-full max-w-lg rounded-3xl flex flex-col animate-fade-in"
          style={{
        ...
        >
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-white font-bold text-lg leading-tight">Checkout Order</h2>
                <p className="text-xs font-medium mt-0.5" style={{ color: '#33A1E0' }}>{cartItems.length} {cartItems.length === 1 ? 'item' : 'items'} in your cart</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          {submitted ? (
            ...
            <div className="flex flex-col items-center justify-center py-14 px-6 gap-5">
              ...
              <div className="text-center space-y-2">
                ...
                <p className="text-gray-400 text-sm max-w-xs leading-relaxed">
                  {isEmail
                    ? 'Your cart order email has been sent to our team. We will contact you shortly to confirm.'
                    : 'Your cart order has been sent to our WhatsApp. Our team will contact you shortly to confirm.'}
                </p>
              </div>
              <button
                onClick={onClose}
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5 px-6 py-6">
              ...
              {/* ── Cart Items Summary ── */}
              <div>
                <label className="block text-xs font-semibold uppercase tracking-widest text-gray-400 mb-2">Items Summary</label>
                <div
                ...
                >
                  {cartItems.map(item => (
                    <div
                      ...
                      style={{ background: 'rgba(51,161,224,0.04)', border: '1px solid rgba(51,161,224,0.08)' }}
                    >
                      <div
                      ...
                      >
                        {item.images?.[0] ? (
                          ...
                          <Package className="w-5 h-5 text-[#33A1E0] opacity-40" />
                        )}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-white font-semibold text-xs leading-tight line-clamp-1">{item.name}</p>
                        <p className="text-gray-400 text-[10px] mt-0.5">Qty: {item.quantity}</p>
                      </div>
                      {item.price > 0 && (
                        <div className="text-right">
                          <p className="text-[#33A1E0] font-bold text-xs">
                ...
                {cartTotal > 0 && (
                  <div
                    className="flex justify-between items-center mt-3 p-3.5 rounded-xl"
                    style={{ background: 'rgba(51,161,224,0.06)', border: '1px solid rgba(51,161,224,0.12)' }}
                  >
                    <p className="text-gray-400 text-xs font-semibold uppercase tracking-wider">Grand Total</p>
                    <p className="text-white font-black text-xl">LKR {cartTotal.toLocaleString()}</p>
                  </div>
                )}
              </div>
